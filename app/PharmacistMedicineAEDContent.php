...
extract($_POST);
 if(isset($Add))
 {
    $sqlq1 = "SELECT * FROM `medicine` WHERE `DrugID` = `$DrugID1`" ;
	...
	$row1= mysqli_fetch_array($que1);
	 ...
	 {
        $sqlq = "INSERT INTO `medicine`(`DrugID`, `Name`, `Price`, `Expiration`) VALUES (`$DrugID1`,`$name2`,`$Price1`,`$Expiration1`)";
 ...
 if(isset($Edit))
 {
    $sqlq1 = "SELECT * FROM `medicine` WHERE `DrugID` = `$DrugID2`" ;
	$que1=mysqli_query($con, $sqlq1);
	$row1= mysqli_fetch_array($que1);
	 if($row1)
	 {  
        $sqlq = "UPDATE `medicine` SET `DrugID`=`$DrugID2`,`Name`='[value-2]',`Price`='[value-3]',`Expiration`='[value-4]'  ";
 ...
 if(isset($Delete))
 {
    $sqlq1 = "SELECT * FROM `medicine` WHERE `DrugID` = `$DrugID3`" ;
	$que1=mysqli_query($con, $sqlq1);
	$row1= mysqli_fetch_array($que1);
	 if($row1)
	 {  
