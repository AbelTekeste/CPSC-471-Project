...
extract($_POST);
 if(isset($Add))
 {
    $sqlq1 = "SELECT * FROM `tasks` WHERE `TaskID`=`$TaskID1` AND `EmployeeSSN`=`$_SESSION[SSN]`" ;
 ...
 if(isset($Edit))
 {
    $sqlq1 = "SELECT * FROM `tasks` WHERE `TaskID`=`$TaskID2`AND `EmployeeSSN`=`$_SESSION[SSN]`"  ;
	$que1=mysqli_query($con, $sqlq1);
	$row1= mysqli_fetch_array($que1);
	 if($row1)
	 {  
        $sqlq = "UPDATE `tasks` SET `PatientID`=`$PatientID2`,`Date`=`$Date2`,`Type`=`$Type2` WHERE `TaskID`=`$TaskID2` AND `EmployeeSSN`=`$_SESSION[SSN]`";
 ...
 if(isset($Delete))
 {
    $sqlq1 = "SELECT * FROM `tasks` WHERE `TaskID`=`$TaskID3` AND `EmployeeSSN`=`$_SESSION[SSN]`" ;
	$que1=mysqli_query($con, $sqlq1);
	$row1= mysqli_fetch_array($que1);
	 if($row1)
	 {  
        $sqlq = "DELETE FROM `tasks` WHERE `TaskID`=`$TaskID3` AND `EmployeeSSN`=`$_SESSION[SSN]`";
        $que=mysqli_query($con, $sqlq);
        if (mysqli_affected_rows($con) > 0) {
